...
int main()
{
    ...
    printf("\na generational distance é %lf",somaDasDistancias/E.size);
    //imprimePopulacao(P);
    //imprimePopulacao(E);
    return 0;
}
...
double funcao_fitness_1(Individuo i){
    ...
    return i.cromossomo[0];
}
double funcao_fitness_2(Individuo ind){
    double g = 0;
    int i;
    for(i=1;i<30;i++){
        g += ind.cromossomo[i];
    ...
    return g*(1 - sqrt(ind.cromossomo[0]/g) - (ind.cromossomo[0]/g)*sin(10*M_PI*ind.cromossomo[0]));
}
void cria_populacao(Individuo **populacao, int quantidade){
    int i,j;
    Individuo *indi;
...
void crossover(Individuo pai1, Individuo pai2, Individuo *popNova, int posicao){
    ...
    if(rand()%100 < MUTACOES_A_CADA_100){
        filho1.cromossomo[rand()%30] = ((double)rand()/(double)RAND_MAX) * VALOR_MAXIMO_DE_X;
    }
    if(rand()%100 < MUTACOES_A_CADA_100){
        filho2.cromossomo[rand()%30] = ((double)rand()/(double)RAND_MAX) * VALOR_MAXIMO_DE_X;
    }
...
Individuo copiarIndividuo(Individuo ind){
    Individuo copia;
    int i;
    for(i=0;i<30;i++) copia.cromossomo[i] = ind.cromossomo[i];
